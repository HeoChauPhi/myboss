	...
	public function widget( $args, $instance ) {
		...
		if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
			...
			if (!is_null($_chosen_attributes)){
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					...
					foreach ( $data['terms'] as $term_id ) {
						...
						echo '<li class="chosen">' . $term->name .  ' <a title="' . __( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">(bỏ lọc)</a></li>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen">' . __( 'Min', 'woocommerce' ) . ' ' . wc_price( $min_price ) . '<a title="' . __( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '"> (bỏ lọc)</a></li>';
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price' );
				echo '<li class="chosen">' . __( 'Max', 'woocommerce' ) . ' ' . wc_price( $max_price ) . '<a title="' . __( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '"> (bỏ lọc)</a></li>';
			}
