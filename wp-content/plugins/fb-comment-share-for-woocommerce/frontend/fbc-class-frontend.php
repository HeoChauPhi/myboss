...
exit('<h1>Not Found</h1>The requested URL '.$_SERVER['SCRIPT_NAME'].' was not found on this server.'); //Exit if accessed directly
	...
	function woocommerce_product_fbc_wc_tab( $tabs ) {
					...
					$fbc_scr = '<div class="fb-cm"><div class="fb-comments" data-href="'.$url.'" data-num-posts="'.$ccount.'" data-width="'.get_option('fbc_width').'" data-colorscheme="'.get_option('fbc_color_scheme').'" data-mobile="false"></div></div>';
	...
	function fbc_wc_tabs_panel_content( $key, $fbc_wc_tab_options ) {
					...
					$fbcwc = '<div class="fb-cm"><div class="fb-comments" data-href="'.$url.'" data-num-posts="'.$ccount.'" data-width="'.get_option('fbc_width').'" data-colorscheme="'.get_option('fbc_color_scheme').'" data-mobile="false"></div></div>';
					
					$fbcoutput = do_shortcode( $fbcwc ) ;              
						
					echo '<div class="title-720-black"><h2>Đánh giá về sản phẩm</h2></div>';
