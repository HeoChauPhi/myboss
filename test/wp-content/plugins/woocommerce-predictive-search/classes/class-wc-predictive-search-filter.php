	...
	public static function get_result_popup() {
		...
		if ($search_keyword != '') {
			...
			if ( $search_products && count($search_products) > 0 ) {
				echo "<div class='ajax_search_content_title'>".__('Sản phẩm', 'woops')."</div>[|]#[|]$search_keyword\n";
				...
				if ( count($search_products) > $row ) {
					...
					$rs_item .= '<div class="more_result" rel="more_result"><a href="'.$link_search.'">'.__('Xem thêm kết quả cho', 'woops').' '.$search_keyword.' <span class="see_more_arrow"></span></a><span>'.__('Xem top', 'woops').' '.$row.' '.__('kết quả', 'woops').'</span></div>';
					echo $rs_item.'[|]'.$link_search.'[|]'.$search_keyword."\n";
				}
			} else {
				echo '<div class="ajax_no_result">'.__('Không tìm thấy kết quả', 'woops').'</div>';
			}
		}
		die();
	}
